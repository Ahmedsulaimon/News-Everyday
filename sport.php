  ...
  <?php  $db = new SQLite3('C:\xampp\htdocs\NewsApplication\data\News_Application.db');
      $sql = "SELECT * FROM news WHERE status = 'approved' AND news_type = 'Sport news'  ORDER BY  upload_date  DESC";
      $stmt = $db->prepare($sql);
  ...
  echo'<form action="sport.php" method="post" class="form-design-for-comment" >';
    echo '<label style="font-weight: bold;" for="lname">comment:</label>';
        ...
        while ($row = $comments_result->fetchArray()) {
            echo '<div class="commentBlock">
                    ...
                    '<form action="inc/delete.php" method="post"><input type="hidden" name="sportsid" value="' . $row['id'] .
