...
$stmt = $db->prepare($sql);
$stmt->bindParam(':newsid',$contentID , SQLITE3_TEXT); 
$result = $stmt->execute();
$fileName = [];
...
<body>
    <?php  
    $db = new SQLite3('C:\xampp\htdocs\NewsApplication\data\News_Application.db');
    $sql = "SELECT * FROM news WHERE status = 'approved' AND news_type = 'events'  ORDER BY upload_date  DESC";
    $stmt = $db->prepare($sql);
    ...
    <?php
    // query all content related to events from database
    foreach ($fileName as &$img) {
        ...
        echo '<form action="events.php" method="post" class="form-design-for-comment">';
        ...
        while ($row = $comments_result->fetchArray()) {
            echo '<div class="commentBlock">
                   ...
                   // $deletecomment = deleteCommentInEvent();
               
                    (  isset($_SESSION['userRole']) && $_SESSION['userRole'] == 'Admin' ? 
                    '<form action="inc/delete.php" method="post"><input type="hidden" name="id" value="' . $row['id'] .
